...
function format_file_name(fileName){
    ...
    if (fileName.length > 20){
        ...
        fileName = fileName.slice(0, (fileName.length - fileExtension.length)).slice(0, 17);
...
function load_file_manager(directoryInfomation = null){
    ...
    for(let i = 0; i < fileNames.length; i++){
        ...
        if (fileType === "image") src = files[fileName].address;
        else src = "static/images/file.png";
        // ---------
    ...
    if (selection.workingdir === workingDir){ 
        ...
        for (let i = 0; i < selection.items.length; i++){
            ...
            $(`.folder[name="${itemName}"]`).css("background-color", "rgba(100, 100, 100, 0.5)");
            $(`.file[name="${itemName}"]`).css("background-color", "rgba(100, 100, 100, 0.5)");
...
function select_item(itemId){
    ...
    if (selection.workingdir != workingDir && selection.workingdir != null) reset_select();
